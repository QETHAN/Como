...
var _como_prototype = {
        ...
        if (typeof value == 'undefined') {
            ...
            if (name == 'opacity') {
				...
				if (win.getComputedStyle) {
					return win.getComputedStyle(el, null).getPropertyValue(hyphenate(name));
				}
    ...
    children: function(n){
        ...
        for(var i = 0, it, il = nodes.length; i < il; i++){
            it = nodes[i];
    ...
    hasClass: function(name){
		if(name && this[0].className){
			return RegExp('\\b' + Como.String.trim(name) + '\\b').test(this[0].className);
		}
		return false;
    },
    ...
    removeClass: function(name){
    	this.each(function(it){
			it = it[0];
    		if(it.className){
				var regexp = RegExp('\\b' + Como.String.trim(name) + '\\b', 'g');
...
var Como = win.Como = function(selector, context){
...
Como.createElement = function(name, obj){
	var el = Como(doc.createElement(name));
	if(obj){
		for(var i in obj){
			el.attr(i, obj[i]);
		}
...
Como.Event = {
	...
	*/
	...
	__e_handlers: {},
	on: doc.addEventListener ? function(el, name, fun){
			if(!el) return;
			if(el.como) el = el[0];
    ...
    un: doc.removeEventListener ? function(el, name, fun){
			if(!el) return;
			if(el.como) el = el[0];
    ...
    out: function(el, name, fun, one){
		...
		var c = Como.Function.bindEvent(callback, win);
        el._Event.out.push({name: name, fun: fun, efun: c});
        Como.Event.on(doc, name, c);
    },
    	...
    	if(el._Event && el._Event.out && el._Event.out.length){
    		...
    		for(var i = 0; i < arr.length ; i ++){
    			if(name == arr[i].name && fun == arr[i].fun){
    				Como.Event.un(doc, name, arr[i].efun);
    				arr.splice(i, 1);
					return;
    			}
    		}
                ...
                case 3:{
                    if(el.tagName.toLowerCase() == 'table' && value.tagName.toLowerCase() == 'tr'){
                    	if(el.tBodies.length == 0){
                    		el.appendChild(doc.createElement('tbody'));
                    	}
...
(function(){
	function getUrl() {
		...
		var t = url.charAt(0);
		if(t == '.' || t == '/'){
			var base = doc.location.pathname;
	...
	Wait: Como.Class.create({
		...
		success: function(name){
			...
			if(this.names.length == 0){
				win.setTimeout(this.callback, 0);
	        ...
	        if (cur.started === undefined) {
	            ...
	            for (var a in cur.attrs) {
	                ...
	                switch (a) {
						...
						case 'scrollLeft':
						case 'scrollTop':
							var val = (this.obj == doc.body) ? (doc.documentElement[a] || doc.body[a]) : this.obj[a];
							if (_attr.by) {
								_attr.value += val;
							}
							cur['last' + a] = val;
	...
	_init: function(){
		...
		if(doc.addEventListener){
			if(Como.Browser.safari){
				var timeout = setInterval(bind(function(){
				...
				doc.addEventListener('DOMContentLoaded', bind(function(){
					this._onloadHook();
				}, this), true);
			}
		} else {
			...
			doc.write('<script onreadystatechange="if (this.readyState==\'complete\') {this.parentNode.removeChild(this);Como.Hook._onloadHook();}" defer="defer" ' + 'src="' + src + '"><\/script\>');
		}
		win.onload = bind(function(){
			this._onloadHook();
		...
		win.onbeforeunload = bind(function(){
...
if(!Como._version || Como._version != 'only'){
	...
	ext(win.Class = {}, Como.Class, false);
